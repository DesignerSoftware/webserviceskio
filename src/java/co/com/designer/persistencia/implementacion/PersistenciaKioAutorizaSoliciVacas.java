...
public class PersistenciaKioAutorizaSoliciVacas implements IPersistenciaKioAutorizaSoliciVacas {
    ...
    public String consultarSecuenciaPorAutorizadorAus(String secEmpleado, String nitEmpresa, String cadena, String esquemaP, int kioModulo) throws Exception {
        String secAutorizador = null;
        String sqlQuery = "select per.secuencia "
                + "from empleados empl, kioautorizadores ka, kioautorizaausentismos kasv, personas per, KioModulos m "
                + "where empl.secuencia = kasv.empleado "
                + "and kasv.kioautorizador = ka.secuencia "
                + "and per.secuencia = ka.persona "
                + "and m.secuencia = ka.kiomodulo "
        ...
        try {
            ...
            query.setParameter(2, secEmpleado);
            secAutorizador = query.getSingleResult().toString();
            System.out.println("PersistenciaKioAutorizaSoliciVacas" + ".consultarSecuenciaPorAutorizadorAus: " + "secAutorizador: " + secAutorizador);
        } catch (Exception e) {
            System.out.println("Error: "+"PersistenciaKioAutorizaSoliciVacas" + ".consultarSecuenciaPorAutorizadorAus: " + e.toString());
        }
        return secAutorizador;
    }
}
